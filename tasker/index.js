...
const TG_TOKEN = '1705702254:AAFQY3aMjnrPKiEga9UUGMj6umEbEyToPiw'
// 1705702254:AAFQY3aMjnrPKiEga9UUGMj6umEbEyToPiw
    ...
    return new Promise((resolve, reject)=>{
        ...
        sheets.spreadsheets.values.get({
            ...
            let makeArray = ()=>{
                ...
                if (rows.length) {
                    rows.map((row) => {
                        responseArray.push({host: row[0], port: row[1], username: row[2], password: row[3]})
                    })
            ...
            err ? resolve(false) : makeArray()
        })
    })
}

const server = process.env.IP || "168.119.49.213"
...
const check = (timeInMins = 0.01)=>{
    let action = (MASS)=>{
        ...
        MASS.forEach((proxy,index)=>{
            ...
            promiseArray.push(new Promise((resolve, reject)=>{
                const task = (mytime=index * 5)=>{
                    setTimeout(()=>{
                        const destination = `http://${server}:${port}/test`
        ...
        Promise.all(promiseArray).then(()=>{
            writeFile('folder/status.txt', prometeusString, (err)=>{
...
const initial = async (delay=0)=>{
    ...
    new Date().getSeconds() > 30 ? delay+=25000 : null
    setTimeout(function(){
        recheck(6)
    }, 10 * 60 * 1000 - delay)
}
...
const newCheck = (ARRAY) => {
    ...
    ARRAY.forEach((element,index)=>{
        ...
        const proxyToKey = `${proxy.host}:${proxy.port}:${proxy.username}:${proxy.password}`
        promiseArray.push(new Promise((resolve, reject)=>{
            axios.get(`http://142.93.171.79:3000/test`, {
                proxy: {
                    host: proxy.host,
                    port: proxy.port,
                    auth: {username: proxy.username, password: proxy.password}
